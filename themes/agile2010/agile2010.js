...
AgileConference.prototype.loadSpeakerInfo = function() { 
    ...
    $.ajax({
        ...
        async: false,
        timeout: 30000,
        success: function(speakersData) {
            localStorage.setItem("speakers", speakersData);
            conference.conferenceSpeakers = $.parseJSON(speakersData);
        }
    });
};
...
AgileConference.prototype.loadSessionInfo = function() {
    ...
    $.ajax({
        ...
        success: function(sessionsData) {
            ...
            conference.conferenceSessions = $.parseJSON(sessionsData);
        }
    });
};
...
AgileConference.prototype.getPrettySpeakersList = function(speakerIDs) {
    ...
    var _this = this;
    return $.map(speakerIDs, function(id, i) { return _this.conferenceSpeakers[id.replace(" ","")].name; }).join(' and ');
}
...
ConferenceDOMBuilder.prototype.buildSpeakerDOM = function(speakerID, speaker) {
    var speakerDiv = $('<div id="'+speakerID+'"></div>');
    speakerDiv.append($('<div class="toolbar"><h1>'+speaker.name+'</h1><a class="button back" href="#">Back</a></div>'));
    speakerDiv.append($('<div class="speaker-info"><img src="themes/agile2010/img/speakers/'+speakerID+'.gif" width="80" height="120" class="speaker-photo"/><div class="speaker-title2">'+speaker.title+'</div>'+speaker.description+'</div>'));
    return speakerDiv;
};
...
ConferenceDOMBuilder.prototype.buildSessionSpeakerList = function(session) {
    ...
    for (var i = 0; i < speakers.length; ++i) {
        ...
        speakerList.append('<li class="arrow speaker-names"><a href="#'+speakers[i]+'">'+speaker.name+'<div class="speaker-title">'+speaker.title+'</div></li>');
    }
    return speakerList;
};

ConferenceDOMBuilder.prototype.buildSessionDOM = function(sessionID, session) {
    var sessionDiv = $('<div id="'+sessionID+'" class="uses_local_data"></div>');
    sessionDiv.append($('<div class="toolbar"><h1>'+session.title+'</h1><a class="button back">Back</a></div>'));
    sessionDiv.append($('div class="topic">'+session.topic+'</div>'));
    sessionDiv.append(this.buildSessionSpeakerList(session));
    sessionDiv.append($('<ul><li style="color:#E0E0E0;">I will attend<span class="toggle go-skip"><input type="checkbox" topic="'+sessionID+'" class="attend-slider"/></span></li></ul>'));
    sessionDiv.append($('<div class="description">'+session.description+'</div>'));
    return sessionDiv;
};
...
ConferenceDOMBuilder.prototype.updateTopicList = function(day, dayDiv) {
    ...
    for (var sessionIndex = 0; sessionIndex < dayTopics.length; sessionIndex++) {
        ...
        var speakers = (session.speakers === null ? [] : session.speakers.split(','));
        topicList.append($('<li><a href="#'+session.id+'" class="topic-link">'+session.title+'</a><span class="speaker-title3">'+this.conference.getPrettySpeakersList(speakers)+'</span><span class="toggle go-skip"><input type="checkbox" topic="'+session.id+'" class="attend-slider"/></span></li>'));
    }
};
